...
import { ArrowLeft, DollarSign, Gift, Lock, Unlock, FileText, X, ChevronDown, Calendar, Plane, Search, Building2, Banknote } from "lucide-react";
...
export default function CreditCardDetails() {
  ...
  const handleCashAdvance = () => {
    executeCashAdvance("freedomUnlimited", "checking", transferAmount);
    setTransferAmount("");
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex justify-between items-center cursor-pointer">
              <div>
                ...
                <span className="font-semibold text-gray-900">{db.accounts.checking.name} (...{db.accounts.checking.mask})</span>
                <p className="text-sm text-gray-500 mt-1">Avail: {formatMoney(db.accounts.checking.balance)}</p>
              </div>
              <ChevronDown className="w-5 h-5 text-gray-400" />
            </div>
            <div className="space-y-2">
              ...
              <label className="flex justify-between items-center bg-white border border-[#0b5cba] ring-1 ring-[#0b5cba] rounded-xl p-4 cursor-pointer">
                ...
                <div className="flex items-center gap-3">
                  <span className="font-semibold text-gray-900">{formatMoney(account.paymentDue)}</span>
                  <div className="w-5 h-5 rounded-full border-4 border-[#0b5cba]"></div>
                </div>
              </label>
              <label className="flex justify-between items-center bg-white border border-gray-200 rounded-xl p-4 cursor-pointer hover:bg-gray-50">
                <span className="font-semibold text-gray-900">Current Balance</span>
                <div className="flex items-center gap-3">
                  <span className="font-semibold text-gray-900">{formatMoney(account.balance)}</span>
                  <div className="w-5 h-5 rounded-full border border-gray-300"></div>
                </div>
              </label>
            </div>
            <button onClick={() => setActiveAction(null)} className="w-full bg-[#0b5cba] text-white font-semibold py-4 rounded-xl mt-4">Review Payment</button>
          </div>
        );
      case "Rewards Hub":
        return (
